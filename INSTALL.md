...
It asks for your sudo password for: apt installs, the `/dev/uinput` udev rule, the `uinput` group change, extracting Ollama to `/usr/local`, creating the `ollama` user and system service, and — in push-to-talk mode — the `hotkeyd` user, `/usr/local/lib/hotkeyd/hotkeyd.py` and its system service. Everything else is under your home directory. Each `==` section ends with green ✔ lines; a yellow `!` is informational; a red ✘ stops the run and says why.
