...
| 1 | CI: GitHub Actions | Every later step needs an automatic safety net independent of the agent's own claims | `.github/workflows/ci.yml`: `pytest -q tests/`, `bash -n install.sh`, `shellcheck install.sh`, `python -m py_compile polish.py dictate`; badge in README | cloud | ready |
